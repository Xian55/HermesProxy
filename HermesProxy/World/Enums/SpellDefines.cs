...
// V3_4_3.54261 SpellCastResult enum — what the WotLK Classic re-release (build 54261)
// client expects. Mirrors CypherCore's SpellConst.cs enum verbatim. Diverges from the
// HermesProxy SpellCastResultClassic enum (synced to V1_14 / V2_5) starting at
// `CantBeSalvaged=16` (Classic still has `CantBeDisenchanted` here) and again at
// `DisabledByPowerScaling=31` which V3_4_3 added — every value at index 16+ shifts.
// Without this dispatch, TC's `NotShapeshift=68` (WotLK enum) was getting name-mapped to
// SpellCastResultClassic.NotShapeshift=89, but 89 in the V3_4_3 client is `NotOnTaxi`
// → spell errors displayed flight-related text ("You are in flight!") for every Bear
// Form / shapeshift cast attempt.
public enum SpellCastResultV343 : uint
{
    Success = 0,
    AffectingCombat = 1,
    AlreadyAtFullHealth = 2,
