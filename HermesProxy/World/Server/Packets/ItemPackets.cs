...
public class InventoryChangeFailure : ServerPacket, ISpanWritable
{
    ...
    // The diagnostic log still reports both conditional fields; the union means only one of
    // them is ever carried, so the other reads as 0 the way the unset field used to.
    private int Level => Variant.Value is InventoryFailureLevelData level ? level.Level : 0;
    private int LimitCategory => Variant.Value is InventoryFailureLimitData limit ? limit.LimitCategory : 0;
