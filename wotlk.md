...
**Pattern G — class-blocker DCs / character-create gating** (severe):
  - **Hunter — total DC on every world enter.** Class-specific hard block. Likely a `PetData` / `PetSpells` create-block layout bug that crashes the V3_4_3 client during the initial world-enter SMSG_UPDATE_OBJECT.
