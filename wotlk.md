...
| Action bar — Always Show Action Bars checkbox persistence | ✅ | ✅ | ⚠️ | Natural account-data round-trip; the `alwaysShowActionBars` CVar is in GlobalConfigCache (type 0) which the V3_4_3 client uploads + reads back **AzerothCore 2026-08-24: does not survive a logout/login cycle.** The CVar itself round-trips — the checkbox is still ticked on the next login — but the client does not apply it, so empty bars stay hidden until the box is re-ticked. Same visible symptom as the Bar 2/3/4/5 row below, and for the same reason: what fails is showing an *empty* bar. |
| Action bar — Bar 2 / 3 / 4 / 5 visibility checkbox persistence | ⚠️ | ❓ | ⚠️ | 2026-05-10 — user must re-tick each session. The V3_4_3 client UI ignores the legacy `bottomLeftActionBar` / `rightActionBar` etc. CVars for these bars (almost certainly uses Edit Mode account-data types 13/14 added in V3_4_4+, which V3_4_3.54261's HermesProxy enum doesn't yet wire). Phase 7 augmenter + Phase 8 timestamp bump inject the legacy CVars defensively (no harm, no help). See the "Action bar visibility & macro persistence (V3_4_3) — May 2026" section below **AzerothCore 2026-08-24: same ⚠️ as TC, with the boundary pinned down.** A bar holding at least one ability survives a logout/login cycle intact; a bar left empty never comes back regardless of the checkbox. So the button contents persist fine and it is purely the empty-bar visibility that is lost, which is consistent with the Edit Mode account-data theory in this note. |
