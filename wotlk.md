...
| 2026-08-28 | **Dark Portal Outland face covers the full width.** Xian called out the Dark Portal workaround in [#125](https://github.com/Xian55/HermesProxy/issues/125#issuecomment-5400283077) (`511e188`). The 3.4.3 client still has no BL-side AreaTrigger and 4356's ActionSet 26557 is dangling, so a hotfix inject of 4352/4354 did not make the client fire `CMSG_AREA_TRIGGER`. The move-hook stays. Outland was a 10-yard sphere in the middle of the frame; it now uses the 3.3.5 DBC box (72×14×54, AC `IsWithinBox`) so left/center/right all port. Arrival pad at `(-248, 922.9, 84)` stays outside. Play-tested both ways on AzerothCore. | (this PR) |
